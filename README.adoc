...
WARNING: This is still being worked on. There is no garantee this will work on
         your system. It is a rice for my own custom needs anyways. It also
         assumes that you have an AMD video card.

== ⬇️ Installing
1. Install the minimal version of Ubuntu server
2. Start your new ubuntu server and clone this repository where you'd like
4. `cd` to this repository and run `./install <absolute path of repository directory>`
...
| Super+Shift+j  | move stack left
| Super+Shift+k  | move stack right
| Super+F1       | toggle audio mixer
| Super+F2       | audio volume down
| Super+F3       | audio volume up
| Super+F4       | audio volume mute
| Super+Shift+m  | toggle htop
| Super+Shift+e  | toggle calculator
|===
...
List internet devices and pick one
...
> ADD_NETWORK
> SET_NETWORK <NETWORK ID> ssid "<ssid>"
...
- [ ] Add mingw gcc compiler, for windows development
- [ ] Refactor scripts
